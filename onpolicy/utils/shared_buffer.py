    ...
    def __init__(self, args, num_agents, obs_space, cent_obs_space, act_space):
        ...
        self.rnn_states = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads,
             num_agents, self.recurrent_N, self.hidden_size),
            dtype=np.float32)
                    ...
                    if self._use_popart or self._use_valuenorm:
                        if self.algo == "mat" or self.algo == "mat_dec":
                            ...
                            delta = rewards_t + self.gamma * \
                                self.masks[step + 1] * value_t_next - value_t
                            gae = delta + self.gamma * \
                                self.gae_lambda * self.masks[step + 1] * gae
                        ...
                        if self.algo == "mat" or self.algo == "mat_dec":
                            ...
                            delta = rewards_t + self.gamma * \
                                self.masks[step + 1] * mean_v_t_next - mean_v_t
    ...
    def feed_forward_generator_transformer(self, advantages, num_mini_batch=None, mini_batch_size=None):
        ...
        if self.available_actions is not None:
            available_actions = self.available_actions[:-
                                                       1].reshape(-1, *self.available_actions.shape[2:])
            available_actions = available_actions[rows, cols]
        value_preds = self.value_preds[:-
                                       1].reshape(-1, *self.value_preds.shape[2:])
        ...
        active_masks = self.active_masks[:-
                                         1].reshape(-1, *self.active_masks.shape[2:])
