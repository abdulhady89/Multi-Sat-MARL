...
class R_MAPPO():
    ...
    def ppo_update(self, sample, update_actor=True):
        ...
        values, action_log_probs, dist_entropy, action_logits = self.policy.evaluate_actions(share_obs_batch,
                                                                                             obs_batch,
                                                                                             rnn_states_batch,
        ...
        dists = [Categorical(acts) for acts in action_logits.logits]

        # Entropy regularization for encouraging exploration within each distribution
        entropies = [dist.entropy() for dist in dists]
        total_entropy = torch.stack(entropies, dim=0).mean()

        # Calculate diversity loss between distributions (pairwise KL divergence)
        diversity_loss = 0
        num_distributions = len(dists)

        for i in range(int(num_distributions/self.num_agents)):
            for j in range(self.num_agents):
                for k in range(j + 1, self.num_agents):
                    kl_div = torch.distributions.kl.kl_divergence(
                        dists[i*self.num_agents+j], dists[i*self.num_agents+k]).mean()
        ...
        if self.num_agents == 1:
            ...
            # Average over pairs
            diversity_loss /= (self.num_agents * (self.num_agents - 1) /
                               2) * (num_distributions/self.num_agents)

        if update_actor:
            actor_loss = policy_loss - self.entropy_coef * \
                total_entropy - self.diversity_coef * diversity_loss
            actor_loss.backward()
        ...
        # critic_loss = value_loss * self.value_loss_coef - self.entropy_coef * \
        #     total_entropy - self.diversity_coef * diversity_loss
        ...
        return value_loss, critic_grad_norm, policy_loss, dist_entropy, actor_grad_norm, imp_weights, actor_loss, diversity_loss
    ...
    def train(self, buffer, update_actor=True):
        ...
        for _ in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                ...
                value_loss, critic_grad_norm, policy_loss, dist_entropy, actor_grad_norm, imp_weights, actor_loss, diversity_loss \
                ...
                train_info['diversity_loss'] += diversity_loss.item()
