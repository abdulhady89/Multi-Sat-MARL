...
class Runner(object):
    ...
    def __init__(self, config):
        ...
        elif self.algorithm_name == "c_ppo":
            from onpolicy.algorithms.c_ppo.centralized_ppo import C_PPO as TrainAlgo
            from onpolicy.algorithms.c_ppo.algorithm.cPPOPolicy import cPPOPolicy as Policy
        else:
            # from onpolicy.algorithms.r_mappo.r_mappo import R_MAPPO as TrainAlgo
            from onpolicy.algorithms.r_mappo.r_mappo_div import R_MAPPO as TrainAlgo
        ...
        if self.algorithm_name == "mat" or self.algorithm_name == "mat_dec":
            self.policy = Policy(
                self.all_args, self.envs.observation_space[0], share_observation_space, self.envs.action_space[0], self.num_agents, device=self.device)
        else:
            self.policy = Policy(
                self.all_args, self.envs.observation_space[0], share_observation_space, self.envs.action_space[0], device=self.device)
