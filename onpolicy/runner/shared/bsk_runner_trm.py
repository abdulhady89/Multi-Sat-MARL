...
class BSKRunner(Runner):
    ...
    def run(self):
        ...
        while n_step < self.num_env_steps:
            ...
            if self.use_linear_lr_decay:
                self.trainer.policy.lr_decay(n_step, self.num_env_steps)
            ...
            if episode % self.log_interval == 0:
                ...
                train_infos["average_episode_rewards"] = np.mean(score)
    ...
    def collect(self, step):
        ...
        # pdb.set_trace()
        actions = np.array(np.split(_t2n(action), self.n_rollout_threads)) if not self.algorithm_name == "c_ppo" else np.array(
            np.split(_t2n(torch.tensor(action)), self.n_rollout_threads))
        action_log_probs = np.array(
            np.split(_t2n(action_log_prob), self.n_rollout_threads)) if not self.algorithm_name == "c_ppo" else np.array(
            np.split(_t2n(torch.tensor(action_log_prob)), self.n_rollout_threads))
        rnn_states = np.array(
