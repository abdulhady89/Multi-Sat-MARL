...
# for baud in {1.0..2.0}; 
for baud in 0.5 1.0 2.0; 
do
    for seed in `seq ${seed_max}`;
    do
        echo "seed is ${seed}:, baud is ${baud}"
        CUDA_VISIBLE_DEVICES=0 python ../../train/train_bsk.py --env_name ${env} --algorithm_name ${algo} --experiment_name ${exp} \
        --scenario_name ${scenario} --constellation_type ${constellation} --num_agents ${num_agents} --seed ${seed} --baud_rate ${baud}\
